...
def main():
	...
	train_batches = preprocess_dataset(train_data_ip, "train", info)
	val_batches = preprocess_dataset(train_data_av, "val", info)
	test_batches = preprocess_dataset(test_data_ip, "test", info)
	
	# Compute data-dependent variables
	train_num_samples = tensorflow.data.experimental.cardinality(train_data_ip).numpy()
	val_num_samples = tensorflow.data.experimental.cardinality(train_data_av).numpy()
